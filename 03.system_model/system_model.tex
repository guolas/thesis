...
\begin{itemize}
    ...
    \item The power available at each of the \gls{bs} will be assumed the same,
        \emph{i.e.} $P_{j, \max} = P_{\max}$ for all $j$.
\end{itemize}
...
\begin{equation} \label{eq:propagation_decomp}
    \HH_{ij} = \RR_{\text{rx}, ij} \Hbar_{ij} \RR_{\text{tx}, ij}
\end{equation}

\noindent
where $\RR_{\text{tx}, ij} \in \C^{t \times t}$ is the spatial correlation at
the transmitter, $\Hbar \in \C^{r \times t}$ is the fading channel, and
$\RR_{\text{rx}, ij} \in \C^{r \times r}$ is the spatial correlation at the
receiver. In the current work, unless stated otherwise, the channel is
considered spatially uncorrelated, both at the transmitter and at the receiver,
that is $\RR_{\text{rx}, ij} = \eye$ and $\RR_{\text{rx}, ij} = \eye$, as in
\cite{karakayali06}.

The small scale characteristics considered are Rayleigh, so that the entries of
$\Hbar_{ij}$ are \gls{iid} complex Gaussian random variables with zero mean and
a variance given by the power path loss between the $j$-th \gls{bs} and the
$i$-th user.

The attenuation that the signal experiments due to the propagation varies
